...
# Cabeçalho: somente Eletromix, sem slogan.
...
# Versão visível perto do atualizador.
...
# Mantém o nome Eletromix fixo no topo, sem substituir pelo nome da loja.
js=re.sub(r'\$\("#storeName"\)\.textContent=[^;]+;', '$('# + '"storeName"' + ').textContent="Eletromix";', js)
# Substitui o render do atualizador para mostrar versão atual e disponível lado a lado.
...
:root{
  --ui-radius:16px;
  --ui-radius-sm:11px;
  --ui-shadow:0 10px 30px rgba(16,24,32,.075);
  --ui-shadow-hover:0 16px 38px rgba(16,24,32,.11);
}
body{
  background:
    radial-gradient(circle at 84% -10%,color-mix(in srgb,var(--accent) 10%,transparent),transparent 32%),
    radial-gradient(circle at 18% 8%,color-mix(in srgb,var(--accent) 5%,transparent),transparent 28%),
    var(--page-bg)!important;
}
header{
  height:68px!important;
  padding:0 24px!important;
  background:color-mix(in srgb,var(--topbar) 94%,#fff 6%)!important;
  border-bottom:1px solid #ffffff14;
  box-shadow:0 8px 30px rgba(5,12,18,.15)!important;
...
.layout{min-height:calc(100vh - 68px)!important;gap:0}
aside{
  width:224px!important;
  top:68px!important;
  height:calc(100vh - 68px)!important;
  padding:16px 10px 20px!important;
  background:color-mix(in srgb,var(--sidebar) 97%,var(--page-bg) 3%)!important;
  border-right:1px solid color-mix(in srgb,var(--border) 82%,transparent)!important;
  box-shadow:8px 0 28px rgba(18,30,40,.035);
}
...
.card,.settings-block{
  ...
  background:color-mix(in srgb,var(--card-bg) 98%,var(--page-bg) 2%)!important;
}
.card{padding:20px!important;margin-bottom:16px!important}.card h3{letter-spacing:-.018em;font-weight:800}.card:hover{border-color:color-mix(in srgb,var(--border) 60%,var(--accent) 10%)!important}
...
.settings-block{padding:22px!important}.nfce-settings-block{position:relative;overflow:hidden}.nfce-settings-block:before{content:"";position:absolute;inset:0 0 auto;height:3px;background:linear-gradient(90deg,var(--accent),color-mix(in srgb,var(--accent) 25%,transparent));opacity:.8}.nfce-info-card{border-radius:13px!important}.nfce-ready-card{border-radius:13px!important}
#toast{border-radius:12px!important;box-shadow:0 14px 34px #0003!important}
