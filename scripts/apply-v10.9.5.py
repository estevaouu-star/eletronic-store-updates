...
# Botão Faltando informação na aba Produtos.
tool='<div class="card"><div class="toolbar"><input id="filtroProdutos" placeholder="Buscar produto..."><select id="filtroStatusProduto"><option value="">Todos</option><option value="ativo">Ativos</option><option value="inativo">Inativos</option></select></div>'
if tool in html:
    html=html.replace(tool,'<div class="card"><div class="toolbar produtos-toolbar"><input id="filtroProdutos" placeholder="Buscar produto..."><select id="filtroStatusProduto"><option value="">Todos</option><option value="ativo">Ativos</option><option value="inativo">Inativos</option></select><button id="filtroInfoProdutoBtn" type="button" class="secondary small">Faltando informação</button></div>',1)
else:
    raise RuntimeError('Toolbar de Produtos não encontrada')
...
# Estado do filtro de informação incompleta.
...
# Aplica o filtro dentro da lista paginada de 25.
...
if old not in js: raise RuntimeError('Filtro renderProdutos não encontrado')
js=js.replace(old,new,1)

# Vendedor obrigatório antes de qualquer início de venda.
...
if old_add not in js: raise RuntimeError('Função add não encontrada')
js=js.replace(old_add,new_add,1)
old_serv='function addServico(id){\n  const s=servicos.find(x=>x.id===id);'
new_serv='function addServico(id){\n  if(!$("#vendedorVenda")?.value)return toast("Selecione o vendedor antes de iniciar a venda.");\n  const s=servicos.find(x=>x.id===id);'
if old_serv not in js: raise RuntimeError('Função addServico não encontrada')
js=js.replace(old_serv,new_serv,1)

# Código de barras também não pode iniciar venda sem vendedor.
...
# Finalização também exige vendedor, mesmo se alguém tentar chamar finish diretamente.
...
print('10.9.5: COBRAR abaixo do total, pagamento flutuante, vendedor obrigatório e filtro Faltando informação restaurado.')