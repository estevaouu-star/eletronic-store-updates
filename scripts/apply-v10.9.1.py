...
js += r'''
// 10.9.1 - Caixa inspirado estruturalmente no Smart PDV + paginação leve de Produtos
(function(){
 ...
 function searchInput(s){return qa('input',s).find(i=>/pesquisar por nome|marca|especificação/i.test((i.placeholder||'')+' '+txt(i.parentElement)))}
 ...
 function paginateProducts(){
   const s=q('#produtos'); if(!s||s.dataset.page10==='1')return; const list=qa('.product',s); if(list.length<=10)return; s.dataset.page10='1'; let page=1; const size=10; const host=list[0].parentElement; const all=list.map(x=>x.cloneNode(true));
   const pager=document.createElement('div');pager.className='products-pager';host.insertAdjacentElement('afterend',pager);
   function render(){const current=qa('.product',host);current.forEach(x=>x.remove()); const start=(page-1)*size; all.slice(start,start+size).forEach(x=>host.appendChild(x.cloneNode(true))); const pages=Math.ceil(all.length/size);pager.innerHTML=''; const prev=document.createElement('button');prev.textContent='‹ Anterior';prev.disabled=page===1;prev.onclick=()=>{page--;render()};pager.appendChild(prev); for(let p=Math.max(1,page-2);p<=Math.min(pages,page+2);p++){const b=document.createElement('button');b.textContent=p;b.className=p===page?'active':'';b.onclick=()=>{page=p;render()};pager.appendChild(b)} const next=document.createElement('button');next.textContent='Próxima ›';next.disabled=page===pages;next.onclick=()=>{page++;render()};pager.appendChild(next)} render();
 }
